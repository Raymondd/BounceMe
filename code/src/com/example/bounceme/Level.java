...
public class Level {
	...
	public Level(int levelNum, int w, int h){
		...
		switch(levelNum){
			case 1:
				slings = new Sling[1];
				slings[0] = new Sling(width/2, (height/3)*2, 200); 
				ball = new Ball(width/2, height/3);
				goal = new Goal(width/2 + 100, height/3 + 100);
				break;
			case 2:
				slings = new Sling[2];
				slings[0] = new Sling(width/3, (height/3)*2, 200); 
				slings[1] = new Sling((width/3)*2, (int) ((height/3)*1.5), 200); 
				ball = new Ball(width/3, height/6);
				goal = new Goal((width/3)*2, height/10 + 100);
				break;
			case 3:
				slings = new Sling[1];
				slings[0] = new Sling(width/5, (height/3)*2, 800);  
				ball = new Ball(width/5, height/3);
				goal = new Goal((width/5)*4, height/3);
				break;
			case 4:
				slings = new Sling[2];
				slings[0] = new Sling(400, 400, 200); 
				slings[1] = new Sling(800, 400, 200);
